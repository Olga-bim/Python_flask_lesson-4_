...
## 4. Front Back
### В веб-разработке часто приходится разделять задачи между серверной частью (backend) и клиентской частью (frontend). В Python и Flask часто используется следующее разделение:

## Backend (серверная часть): Реализована на Python и отвечает за обработку логики приложения, работу с базами данных, выполнение серверных вычислений и генерацию ответов.
## Frontend (клиентская часть): Реализован на HTML, CSS и JavaScript и отвечает за отображение данных пользователю и взаимодействие с ним.
## Разделение Backend и Frontend
## Backend (Python)
## Обработка запросов и логика: Серверная часть отвечает за обработку входящих запросов от клиента (браузера или другого клиента), выполнение бизнес-логики и работу с данными.
## Формат ответа: Сервер может возвращать данные в различных форматах, таких как HTML, JSON, XML и другие.
## Frontend (HTML, CSS, JavaScript)
## Отображение данных: Клиентская часть отвечает за отображение данных, полученных от сервера, и предоставляет интерфейс для взаимодействия с пользователем.
## Обработка пользовательского ввода: Frontend может выполнять действия на стороне клиента, такие как валидация формы, взаимодействие с пользователем и динамическое обновление контента.
## Форматы ответа: HTML vs JSON
### HTML
    Описание: HTML (HyperText Markup Language) используется для описания структуры веб-страниц. Это формат, который браузеры могут интерпретировать и отображать.
    Пример: Веб-страницы, содержащие текст, изображения, таблицы и формы.
    Когда использовать: HTML подходит для серверной генерации полноценных веб-страниц. Например, при рендеринге динамических страниц с помощью Flask и Jinja2.
...
## В шаблоне вы можете использовать переменные, переданные из серверного кода, а также выполнять логические операции, циклы и другие действия для динамического формирования содержимого.
